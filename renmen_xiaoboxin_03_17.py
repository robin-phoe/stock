...
def core(df,date):
    ...
    df_avg_60 = df.groupby(['stock_id'])['close_price'].rolling(60).mean()
    # print('df_avg_60:', df)
    # df_avg_60['cha_60'] = df_avg_60['close_price'] #- df_avg_60.groupby(['stock_id'])['close_price'].shift(20)
    # df_avg_60_s = df_avg_60.shift(20)
    # print('df_avg_60:',df_avg_60_s.head(100),df_avg_60.head(100))
    ...
    #merge 60日均线
    df = pd.merge(df, df_avg_60, how='left', on=['stock_id','trade_date'])
    df.rename(columns={'close_price_x':'close_price','close_price_y': 'avg_60'}, inplace=True)
    ...
    # print('df bais:',df)
    #计算近20日 20日均线走向 及60日均线走向 0.97 为0点
    df['tendency_20'] = df['avg_low'] / df.groupby('stock_id')['avg_low'].shift(20)
    df['tendency_60'] = df['avg_60'] / df.groupby('stock_id')['avg_60'].shift(20)
    #截取近20日数据
    ...
    # 计算20日线走向分数
    df['tendency_20_grade'] = df['tendency_20'].apply(lambda x: 10000 if x>0.97 else 0)
    # 计算60日线走向分数
    df['tendency_60_grade'] = df['tendency_60'].apply(lambda x: 10000 if x > 0.97 else -5000)
    #计算分数
    df['grade'] = df['avg_5_flag'] + df['avg_low_flag'] + df['tendency_20_grade'] + df['tendency_60_grade']
...
def main(date,h_tab):
    ...
    start_t = (date_time - datetime.timedelta(days=180)).strftime('%Y-%m-%d')
    # day_delta = 40
...
if __name__ == '__main__':
    date ='2021-03-18'#'2021-02-01' #'2021-01-20'
    main(date, h_tab = '1')
    # run(date)